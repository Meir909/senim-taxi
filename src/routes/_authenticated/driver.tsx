...
function DriverHome() {
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(push, (e) => console.warn("geo error", e), {
  ...
  async function setRideStatus(status: Ride["status"], pin?: string) {
    ...
    if (status === "in_progress") {
      const { error } = await supabase.rpc("start_ride_with_pin", {
      ...
      <Card className="flex items-center justify-between gap-3 p-5">
        <div className="min-w-0">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1 text-xs text-muted-foreground">
            Подтверждение:{" "}
            <Badge variant={driver.verification === "approved" ? "default" : "secondary"}>
              {VERIFICATION[driver.verification] ?? driver.verification}
            </Badge>
          </div>
        </div>
        <Switch
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0 text-sm">
                      ...
                      <div className="truncate text-muted-foreground">
                        → {o.rides?.dropoff_address || "Точка B"}
                      </div>
                      {o.rides?.tariff === "kids" && (
...
function DriverMap({
  ...
  const target = useMemo(() => {
    ...
    return activeRide.status === "in_progress"
      ? {
          ...
          kind: "B" as const,
          title: "Маршрут до точки B",
          subtitle: "Конечная точка поездки",
        }
      : {
          lat: activeRide.pickup_lat,
          lng: activeRide.pickup_lng,
          kind: "A" as const,
          title: "Маршрут до точки A",
          subtitle: "Точка подачи пассажира",
        };
  }, [activeRide]);
  ...
  useEffect(() => {
    if (!pos || !target) {
      ...
      setRouteError(pos ? "Маршрут недоступен" : "Ожидаем геолокацию водителя…");
      return;
    }
    let cancelled = false;
    const load = async () => {
      try {
        ...
        const r = await fetchRoute({
          data: {
            ...
            dropoff: { lat: target.lat, lng: target.lng },
          },
        });
        if (cancelled) return;
        ...
        if (!cancelled) {
          setRoute(null);
          setRouteError("Не удалось обновить маршрут");
        }
      } finally {
        ...
        <Card className="p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-lg font-semibold">{target.title}</div>
              <div className="text-sm text-muted-foreground">{target.subtitle}</div>
            </div>
            <Badge variant={target.kind === "A" ? "secondary" : "default"}>
              Точка {target.kind}
            </Badge>
          </div>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-3">
            <RouteStat
              label="Статус"
              value={
                routeLoading ? "Обновляем…" : route ? "Маршрут активен" : (routeError ?? "Ожидание")
              }
            />
            <RouteStat
              label="Дистанция"
            ...
            <RouteStat
              label="Время"
              value={route ? `~${Math.max(1, Math.round(route.duration_s / 60))} мин` : "—"}
            />
          </div>
        </Card>
      )}
      ...
      <div className="relative overflow-hidden rounded-xl border">
        ...
        {target && (
          <div className="pointer-events-none absolute left-2 top-2 rounded-md bg-background/90 px-2.5 py-1.5 text-xs font-medium shadow">
            {routeLoading
              ? `Строим маршрут до точки ${target.kind}…`
              : route && route.distance_m > 0
                ? `До точки ${target.kind}: ${route.distance_m < 1000 ? `${Math.round(route.distance_m)} м` : `${(route.distance_m / 1000).toFixed(1)} км`} · ≈ ${Math.max(1, Math.round(route.duration_s / 60))} мин`
                : (routeError ?? `Маршрут до точки ${target.kind} недоступен`)}
          </div>
        )}
      </div>
    </div>
    ...
    <Card className="space-y-3 p-5">
      ...
      {ride.tariff === "kids" && ride.status === "driver_arrived" && (
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-3">
          ...
          <div className="mt-1 text-xs text-muted-foreground">
            Поездка начнётся только после правильного 4-значного кода.
          </div>
          <Input
