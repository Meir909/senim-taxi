...
<<<<<<< HEAD
  ...
  useEffect(() => {
    void supabase
      ...
      .maybeSingle()
      .then(({ data }) =>
        setBlocked({ at: data?.blocked_at ?? null, reason: data?.blocked_reason ?? null }),
      );
  }, [req.user_id]);
...
function DriverCard({
  ...
  const [signed, setSigned] = useState<Record<string, string>>({});
  const docPathsKey = useMemo(() => docs.map((d) => d.file_path).join("|"), [docs]);

  useEffect(() => {
    const paths = [profile?.selfie_path, ...docs.map((d) => d.file_path)].filter(
      Boolean,
    ) as string[];
    if (paths.length === 0) return;
    void supabase.storage
      ...
      .then(({ data }) => {
        const map: Record<string, string> = {};
        data?.forEach((x) => {
          if (x.path && x.signedUrl) map[x.path] = x.signedUrl;
        });
        setSigned(map);
      });
  }, [driver.id, profile?.selfie_path, docs, docPathsKey]);
          ...
          variant={
            driver.application_status === "approved"
              ...
              : driver.application_status === "rejected"
                ? "destructive"
                : driver.application_status === "needs_reupload"
                  ? "outline"
                  : "secondary"
          }
        >
...
<<<<<<< HEAD
          ...
          >
            <img
              src={signed[profile.selfie_path]}
              alt="Селфи"
          ...
          (kind) => {
            ...
            return (
              <DocReviewRow
                ...
                kind={kind}
                doc={doc}
                signedUrl={doc ? signed[doc.file_path] : undefined}
                onChanged={onChanged}
              />
            );
          },
...
<<<<<<< HEAD
          <img
            src={url}
            alt={label}
            className="aspect-square w-40 rounded-md border border-border object-cover"
          />
        </a>
      ) : (
