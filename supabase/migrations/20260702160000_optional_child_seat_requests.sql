ALTER TABLE public.rides
  ADD COLUMN IF NOT EXISTS requires_child_seat boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  WHERE NOT EXISTS (
    ...
    AND (
      COALESCE(v_ride.requires_child_seat, false) = false
      OR COALESCE(d.child_seat, false) = true
    )
...
BEGIN
  ...
  IF COALESCE(v_ride.requires_child_seat, false) = true
     AND COALESCE(v_driver.child_seat, false) = false THEN
    UPDATE public.ride_offers SET status='cancelled', responded_at=now() WHERE id=_offer_id;
    RAISE EXCEPTION 'Для этого заказа требуется детское кресло';
  END IF;
